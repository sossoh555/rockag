function alt = altitude(mE,mp,Mpay)
global m0 T A Cd Re hgr_turn tf md %rh0 H0
global Isp
% Launch Site: Guiana Space Center
Alt = 1;              %[m] Alt above sea level

% VEGA Rocket
m_stage_gross = mE;% 1st, 2nd,3d
% First stage(Solid Fuel)   
m_prop = sum(mp);      % *GA* {300,000 - 700,000}[kg] Propellant mass
      % *CONSTANT*[s]  Specific impulse
d      = 3;           % *GA* {2 - 6} [m]  Diameter 
g0     = 9.81;        % [m/s^2] Constant at its sea-level value
m0  = sum(mE)+sum(mp)+Mpay;         % *GA* { [kg] Initial mass
...
tburn = 250;          % [s] Fuell burn time, first stage
